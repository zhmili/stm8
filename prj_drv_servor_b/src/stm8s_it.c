 ...
 INTERRUPT_HANDLER(TIM2_CAP_COM_IRQHandler, 14)
{
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
   if(TIM2_GetITStatus(TIM2_FLAG_CC1) != RESET)// r
   {
     ULT.once_ready = 0;
     TIM2_ClearFlag(TIM2_FLAG_CC1);
     ULT.cap_val1 = TIM2_GetCapture1();
...
#if PRINT_ON
     printf("come in tim2 cap int cc1");
#endif
   }
   else if(TIM2_GetITStatus(TIM2_FLAG_CC2) != RESET) // d
   {
     TIM2_ClearFlag(TIM2_FLAG_CC2);
     ULT.cap_val2 = TIM2_GetCapture2();
...
#if PRINT_ON
     printf("come in tim2 cap int cc2");
#endif
   }
}
#endif /*STM8S903*/
...
#if defined (STM8S208) || defined(STM8S207) || defined(STM8S103) || defined(STM8S003) || defined (STM8AF62Ax) ||\
